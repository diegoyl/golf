...
// BALL STRIKING:
  ...
  // club selection, only select putter if on green
  // avatar gets flipped sometimes
  // adjust club positioning/rot by club?
  // push/pull mii before shot like in wii when you press A so ball isnt covered
...
window.addEventListener('keydown', (event) => {
  ...
  if (event.key === 'ArrowLeft') {
    ...
    GAME_MANAGER.changeClub(clubName)
  }
});
...
const CLUB_PROFILES = { // what was 3wood angle
  "driver" : {model:false, name:"Driver",  launchAngle: 12 ,   maxPower: 14,     minSwingSpin: 0,    maxSwingSpin: 1,     spinRate: 1,     drag: 0.00107,  carry: 275},
  "wood" :   {model:false, name:"3 Wood",  launchAngle: 11.2 , maxPower: 11.97,  minSwingSpin: 0,    maxSwingSpin: 0.9,   spinRate: 2.25,  drag: 0.00121,  carry: 243},
  "3i" :     {model:false, name:"3 Iron",  launchAngle: 12.3 , maxPower: 11.62,  minSwingSpin: 0,    maxSwingSpin: 0.9,   spinRate: 2.44,  drag: 0.00145,  carry: 212},
  "5i" :     {model:false, name:"5 Iron",  launchAngle: 13.3 , maxPower: 9.14,   minSwingSpin: 0,    maxSwingSpin: 0.9,   spinRate: 4.32,  drag: 0.00145,  carry: 194},
  "7i" :     {model:false, name:"7 Iron",  launchAngle: 16.1 , maxPower: 7.42,   minSwingSpin: 0.2,  maxSwingSpin: 0.9,   spinRate: 5.22,  drag: 0.00145,  carry: 172},
  "9i" :     {model:false, name:"9 Iron",  launchAngle: 20.4 , maxPower: 5.75 ,  minSwingSpin: 0.34, maxSwingSpin: 1,     spinRate: 7.2,   drag: 0.00145,  carry: 148},
  "gap" :    {model:false, name:"56°",     launchAngle: 26.7 , maxPower: 4.9,    minSwingSpin: 0.51, maxSwingSpin: 1,     spinRate: 8.35,  drag: 0.00162,  carry: 120},
  "lob" :   {model:false, name:"Lob Wedge", launchAngle: 30.5 ,maxPower: 4.9 ,   minSwingSpcin: 0.32, maxSwingSpin: 1,     spinRate: 13,    drag: 0.00238,  carry: 92},
  "putter" : {model:false, name:"Putter", launchAngle:  0.0 ,  maxPower: 3.0 ,   minSwingSpin: 0.2,  maxSwingSpin: 1,     spinRate: 0.0,   drag: 0.0,      carry: 60},
}
...
const RESTING_TIME = 200 // # frames it stays on resting position after shot is done
...
class CamManager {
  ...
  setTeeCam(direction) {
    ...
    const cameraPos = new THREE.Vector3().copy(ball.position).add(direction.clone().multiplyScalar(cameraDistance));
    CAMERA.position.set(
      cameraPos.x, 
    ...
    CAMERA.lookAt(new THREE.Vector3(HOLE_MANAGER.pinPos.x, HOLE_MANAGER.pinPos.y +yLookOffset , HOLE_MANAGER.pinPos.z));
  }
  ...
  aimCam(aimAngle) {
    ...
    const baseDirection = new THREE.Vector3().subVectors(pinVec, ball.position).normalize();
    ...
    const cameraPosition = ball.position.clone()
    cameraPosition.x += -cameraDistance * aimDirection.x;
    cameraPosition.z += -cameraDistance * aimDirection.z;

    cameraPosition.y += yCamOffset;


    CAMERA.position.copy(cameraPosition);


    const distanceToHole = ball.position.distanceTo(pinVec);
    const lookTarget = ball.position.clone().add(aimDirection.clone().multiplyScalar(distanceToHole))
      .add(new THREE.Vector3(0, yLookOffset, 0)); // add height
  ...
  update () {
    if (this.tracking){
      if (ball.reachedApex) {
        ...
        const minY = getHeightAt(camX,camZ) + 2;
        if (camY > minY + 0.2) {
      ...
      if (this.restTime < RESTING_TIME) {
        ...
        const minFOV = 15;
        if (CAMERA.fov > 15.2) {
          const zoom = (CAMERA.fov - minFOV) * .977 + minFOV;
          this.setFOV(zoom);
        }
      } else {
...
class GameManager {
  ...
  async loadHole() {
    ...
    terrainMesh = await HOLE_MANAGER.loadTerrain();
    console.log("$$$$$$$$$$$$$$$$$$$$$$\nFINISHED TERRAIN\n-------------------------")
...
class HoleManager {
  ...
  if (heightData.length !== vertexCount) {
    ...
    return new Promise((resolve, reject) => {
        ...
        (gltf) => {
            ...
            const maxDim = Math.max(size.x, size.y, size.z);
            const scale = OPT_SIZE / maxDim;
            const ySmash = .6667
            terrain.scale.set(scale, scale*ySmash, scale);

            // const center = new Vector3();
            // box.getCenter(center);
            // terrain.position.sub(center); // Move model to origin
            // terrain.rotation.set(0, Math.PI, 0);
             terrain.position.y += 135; // 🔼 move it up by 50 units
            ...
            resolve(terrain); // ✅ resolve once loaded
        ...
        (xhr) => {
            ...
            else if (percent > lastXHR || percent == 100){
                console.log(percent + '% loaded');
                lastXHR += 10;
            }    
        },
  ...
  setupShot() {
    this.holeDistance = ball.position.distanceTo(this.getPinPos);
    ...
    if (currentClub.name != CLUB_PROFILES[bestClub]["name"]) {
      GAME_MANAGER.changeClub(bestClub);
    }
    ...
    const avOffset = perpendicularDir.multiplyScalar(distFromBall);

    // Final avatar position
    const avX = ball.position.x + avOffset.x;
    const avZ = ball.position.z + avOffset.z; // y cus vec2
  ...
  processUserSwing(userSwingPower, userSwingSpin) {
    console.log("processUserSwing with ")
...
const aimIncrement = 0.5
document.getElementById('aimL').addEventListener('click', (e) => { 
  HOLE_MANAGER.changeAim( aimIncrement )
});
document.getElementById('aimR').addEventListener('click', (e) => { 
  HOLE_MANAGER.changeAim( -aimIncrement )
});
